...
- `NEXT_PUBLIC_PADDLE_CLIENT_TOKEN`: Paddle client-side token (`live_...`) from Paddle > Developer Tools > Authentication.
- `NEXT_PUBLIC_PADDLE_PRICE_ID`: Paddle Price ID (`pri_live_...`) for the $99/month product from your live catalog.

## Paddle checkout (Cloudflare Workers)

Worker setup is in [workers/paddle](workers/paddle). It verifies Paddle webhook signatures and handles fulfillment events.
Checkout is opened entirely client-side via Paddle.js — no backend order creation needed.
